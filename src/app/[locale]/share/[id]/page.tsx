...
export default function SharePage() {
    ...
    return (
        <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
            {/* Result Info Header with Action Hub */}
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 p-6 rounded-2xl shadow-sm relative group">
                ...
                <div className="space-y-4 flex-1">
                    ...
                    <h1 className="text-2xl md:text-3xl font-black tracking-tight text-slate-900 dark:text-white break-all leading-tight">
                        {snapshot.target_host}
                    </h1>
                </div>

                {/* Conversion Action Hub */}
                <div className="flex flex-col sm:flex-row items-center gap-3 relative z-10 w-full lg:w-auto">
                    <Link
                        href={`/?tab=${isIpInfo ? 'info' : snapshot.check_type}&host=${snapshot.target_host}`}
            ...
            <AdSlot slotType="share_content" className="mt-8" />

            <div className="text-center pt-8">
                <p className="text-xs text-slate-400">
                    Check provided by <span className="font-bold text-indigo-500">{process.env.NEXT_PUBLIC_SITE_NAME || 'CheckHost'}</span> • Free global network diagnostics
                </p>
            </div>
        </div>
    );
